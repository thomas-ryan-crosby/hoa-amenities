-- Multi-Community Migration SQL Script (Re-runnable Version)
...
-- Step 8: Create production community for existing data
...
-- Step 10: Get sanctuary community ID and assign users
DO $$
DECLARE
...
BEGIN
  ...
  -- Assign all existing users to sanctuary community with their current role
  ...
  -- Assign all existing amenities to sanctuary community
  UPDATE amenities 
  SET "communityId" = sanctuary_id
  WHERE "communityId" IS NULL;
  
  -- Assign all existing reservations to sanctuary community
  UPDATE reservations 
  SET "communityId" = sanctuary_id
  WHERE "communityId" IS NULL;
END $$;

-- Step 11: Make community_id NOT NULL (after data migration)
...
-- Step 12: Update unique constraint on amenities name to be per-community
...
-- Verification queries (run these separately after COMMIT to verify)
